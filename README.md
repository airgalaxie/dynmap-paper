...
The bundled MySQL/MariaDB standalone PHP endpoints are kept for external Apache/Nginx/PHP deployments. They have been updated for the current PHP 8.5 runtime style while keeping the original endpoint names and request parameters:
...
The PHP code now uses strict types, `__DIR__` includes, typed helper functions, `http_response_code`, consistent JSON error responses for JSON endpoints, prepared statements, `utf8mb4`, and path validation. `MySQL_config.php` is still generated by Dynmap and remains compatible with the original variable names.
